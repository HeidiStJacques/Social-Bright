...
export default function PlanOfCarePageADL({ form, setForm, adlScore }) {
  const {
    ...
    levelOfCareLabel = 'Level of Care',
  } = form.tenantSettings || {};
  ...
  return (
    <section className="bg-white text-sm text-black p-4 sm:p-6 my-6 rounded-2xl shadow-md space-y-6">
      <h2 className="text-lg font-bold">ADL Score & {programName} Eligibility</h2>
      ...
      <div>
        <label className="font-semibold block mb-1">
          Does participant meet {programName} {levelOfCareLabel}?
        </label>
        <input
          type="text"
          readOnly
      ...
      <div>
        <label className="font-semibold block mb-1">{programName} Redetermination Date</label>
        <input
          type="date"
